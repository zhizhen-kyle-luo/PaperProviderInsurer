...
When both agents receive no guidance, the outcome collapsed to complete denial: the default provider requested only 2 lines (drug + IV admin), both subject to step-therapy, and the default insurer denied both. The end result matches that of mutual defection (DP\_DI: zero services approved), but the mechanism differs in instructive ways: in NP\_NI, a narrow request met literal denial and triggered persistent appeals, whereas in DP\_DI, an inflated request met blanket rejection. The default--default pair thus reaches the same deadlock as mutual defection, but through bureaucratic attrition rather than overt strategic aggression.
